...
const { readAllPizzas, readOnePizza, createOnePizza, deleteOnePizza, updateOnePizza } = require('../models/pizzas');
...
*/
router.get('/', (req, res) => {
  const allPizzas = readAllPizzas(req.query.order);
...
router.get('/:id', (req, res) => {
  const pizza = readOnePizza(req.params.id);
...
router.patch('/:id', authorize, isAdmin, (req, res) => {
  ...
  const updatedPizza = updateOnePizza(req.params.id, req.body);
